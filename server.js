...
function startRestreaming(streamKey, rtmpPath, platforms) {
  ...
  for (const platform of platforms) {
    ...
    // Kick's ingest runs on Amazon IVS infrastructure (the returned host is
    // *.global-contribute.live-video.net), whose documented RTMPS format is
    // rtmps://<endpoint>:443/app/<stream-key> — the /app/ segment is
    // required by AWS IVS itself, not something Kick's API response
    // includes. Twitch's rtmpUrl already has its own /app baked in
    // (rtmp://live.twitch.tv/app), which is why only Kick needed this.
    // Unconfirmed by an actual ffmpeg error (it printed nothing before
    // dying), but grounded in AWS IVS's documented ingest format — worth
    // testing before chasing anything else.
    const destUrl = platform.name === 'Kick'
      ? `${platform.rtmpUrl}/app/${platform.streamKey}`
    ...
    ffmpeg.stderr.on('data', (data) => {
      ...
      if (!msg) return;
      // Previously only logged lines containing "Connection refused" or
      // "Failed to" — that silently swallowed every other ffmpeg error
      // (protocol errors, TLS handshake failures, "Unknown protocol", etc.),
      // which is why Kick's instant crash produced zero visible output.
      // Log everything at a bounded size instead.
      console.warn(`[Restream/${platform.name}] ${msg.substring(0, 300)}`);
    });
